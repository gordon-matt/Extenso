...
{
    private static readonly Regex GroupRegex = new(@"^group\[(\d*)\]\[(field|dir)\]$", RegexOptions.IgnoreCase);
    private static readonly Regex GroupAggregateRegex = new(@"^group\[(\d*)\]\[aggregates\]\[(\d*)\]\[(field|aggregate)\]$", RegexOptions.IgnoreCase);
    ...
    private static IEnumerable<GroupObject> GetGroupObjects(NameValueCollection queryString, IEnumerable<string> requestKeys)
    {
        ...
        foreach (var groupObject in result)
        {
            ...
            string aggregateKey = $"group[{groupObject.Key}][aggregates]";
