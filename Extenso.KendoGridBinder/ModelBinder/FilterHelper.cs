...
public static class FilterHelper
{
    ...
    */
    ...
    private static FilterObject Map(Filter filter) => new()
    {
        Field1 = filter.Field,
    ...
    private static FilterObjectWrapper GetFilterObjects(NameValueCollection queryString, IList<string> filterKeys, string filterLogic)
    {
        ...
        foreach (int index in GetIndexArr(fieldKeys))
        {
            ...
            string field1 = group.First(g => g.Contains("field"));
            string operator1 = group.First(g => g.Contains("operator"));
            string value1 = group.First(g => g.Contains("value"));
            string ignoreCase1 = group.FirstOrDefault(g => g.Contains("ignoreCase"));
            ...
            if (group.Count is 6 or 8)
            {
                string field2 = group.Last(g => g.Contains("field"));
                string operator2 = group.Last(g => g.Contains("operator"));
                string value2 = group.Last(g => g.Contains("value"));
                string ignoreCase2 = group.LastOrDefault(g => g.Contains("ignoreCase"));
    ...
    private static IEnumerable<int> GetIndexArr(IEnumerable<string> fieldKeys)
    {
        ...
        {
            int index = GetFilterIndex(fieldKey);
    ...
    {
        string[] splitArr = qString.Split('[');

        foreach (string s in splitArr)
        {
            string strippedVal = s.Replace("]", "");
    ...
    private static string GetFilterLogic(NameValueCollection queryString, IEnumerable<string> filterKeys, int index, string type)
    {
        ...
        foreach (string fieldKey in fieldKeys)
        {
            int filterIndex = GetFilterIndex(fieldKey);
