...
public class FakeAuthorizationService : IAuthorizationService
{
    ...
    public void SetAuthorizationState(AuthorizationState state) => currentState = state;
    ...
    public void SetPolicies(IEnumerable<string> policies) => supportedPolicies = policies;

    /// <summary>
    /// Sets the supported roles in the Authorization service for current user.
    /// </summary>
    /// <param name="roles">List of supported roles.</param>
    public void SetRoles(IEnumerable<string> roles) => supportedRoles = roles ?? Array.Empty<string>();
    ...
    public Task<AuthorizationResult> AuthorizeAsync(ClaimsPrincipal user, object? resource, IEnumerable<IAuthorizationRequirement> requirements)
    {
        ...
        result = Array.TrueForAll(requirementsArray, p => p is DenyAnonymousAuthorizationRequirement)
            ? currentState == AuthorizationState.Authorized
                ? AuthorizationResult.Success()
                : AuthorizationResult.Failed()
            : Array.TrueForAll(requirementsArray, p => p is RolesAuthorizationRequirement)
                ? VerifyRequiredRoles(requirementsArray)
                : supportedPolicies is not null ? VerifyRequiredPolicies(requirementsArray) : AuthorizationResult.Failed();
    ...
    private AuthorizationResult VerifyRequiredPolicies(IReadOnlyCollection<IAuthorizationRequirement> requirements) => supportedPolicies.IsNullOrEmpty() || requirements.IsNullOrEmpty()
            ? AuthorizationResult.Failed()
            : requirements.OfType<TestPolicyRequirement>().Any(req => supportedPolicies.Contains(req.PolicyName, StringComparer.Ordinal))
            ? AuthorizationResult.Success()
            : AuthorizationResult.Failed();
