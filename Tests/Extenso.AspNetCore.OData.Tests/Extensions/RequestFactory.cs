...
public static class RequestFactory
{
    ...
    public static HttpRequest Create(Action<ODataOptions> setupAction) => Create("Get", "http://localhost", setupAction);
    ...
    {
        var request = Create("Get", "http://localhost/", setupAction);
    ...
    public static HttpRequest Create(string method, string uri, IEdmModel model)
    {
        ...
        var request = Create(method, uri, setupAction: null);
        var feature = request.ODataFeature();
        feature.RoutePrefix = "";
        feature.Model = model;
        return request;
    ...
    public static TKey GetKeyFromLinkUri<TKey>(this HttpRequest request, Uri link)
    {
        ...
        var uriParser = new ODataUriParser(model, new Uri(serviceRoot), new Uri(link.LocalPath, UriKind.Relative),
            request.GetRouteServices());

        var odataPath = uriParser.ParsePath();

        if (odataPath.Where(p => p is KeySegment).FirstOrDefault() is not KeySegment keySegment || !keySegment.Keys.Any())
        {
