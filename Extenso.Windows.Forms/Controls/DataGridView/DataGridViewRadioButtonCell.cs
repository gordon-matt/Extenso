...
public class DataGridViewRadioButtonCell : DataGridViewComboBoxCell, IDataGridViewEditingCell
{
    ...
    private const int DATAGRIDVIEWRADIOBUTTONCELL_mouseLocationBottomScrollButton = -2;

    // codes used for the mouseLocationCode static variable:
    private const int DATAGRIDVIEWRADIOBUTTONCELL_mouseLocationGeneric = -3;

    // mouse is over bottom scroll button
    private const int DATAGRIDVIEWRADIOBUTTONCELL_mouseLocationTopScrollButton = -1;

    private static int mouseLocationCode = DATAGRIDVIEWRADIOBUTTONCELL_mouseLocationGeneric;

    private CurrencyManager dataManager;

    private bool dataSourceInitializedHookedUp;
    ...
    // Indicates whether the cell handled the key down notification
    private bool mouseUpHooked;

    // Index of the focused radio button entry
    private int pressedItemIndex;

    // Maximum number of radio buttons displayed by the cell
    private int selectedItemIndex;

    // Index of the currently selected radio button entry
    // Index of the currently pressed radio button entry
    // Indicates whether the DataSource's Initialized event is listened to
    private bool valueChanged;

    // Property descriptor for the DisplayMember property
